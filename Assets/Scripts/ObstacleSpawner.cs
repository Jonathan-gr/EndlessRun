...
public class ObstacleSpawner : MonoBehaviour
{
    ...
    [Range(0f, 1f)] public float shieldSpawnChance = 0.05f; // 5%
    // Remaining ~55% will be obstacles
    ...
    void SpawnRandomObject()
    {
        ...
        // 2. Shield
        else if (randomVal < coinSpawnChance + shieldSpawnChance)
        {
            Vector3 spawnPos = new Vector3(xPos, coinHeight, spawnZ);
            Instantiate(shieldPrefab, spawnPos, Quaternion.identity);
        }
        // 3. Log
        else if (randomVal < coinSpawnChance + shieldSpawnChance + logSpawnChance)
        {
